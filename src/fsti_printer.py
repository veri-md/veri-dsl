...
class FStarPrinter:
    ...
    def _print_val(self, d: ValDecl):
        ...
        elif d.contract.requires:
            # Pure with requires but no ensures needs a minimum ensures clause
            self._line(f"  (ensures (fun result -> True))")
    ...
    def _print_imported(self, d: ImportedDecl):
        """ImportedDecl → F* assume val with contract."""
        parts = ["val", f"{_to_fstar_name(d.name)}:"]
        for p in d.params:
            parts.append(f" {p.name}:{self._type(p.typ)} ->")
        ret = self._type(d.return_type) if d.return_type else 'unit'
    ...
    def _print_extern(self, d: ExternDecl):
        """ExternDecl → F* assume val with contract."""
        parts = ["val", f"{_to_fstar_name(d.name)}:"]
        for p in d.params:
            parts.append(f" {p.name}:{self._type(p.typ)} ->")
        ret = self._type(d.return_type) if d.return_type else 'unit'
    ...
    def _expr(self, expr: Expr) -> str:
        ...
        if isinstance(expr, BinOp):
            ...
            if op == '+':
                if isinstance(expr.left, App) and isinstance(expr.left.func, Var) and expr.left.func.name == 'Cons':
                    ...
                    return f"{h} :: {t}"
            if op == 'in':
                return f"List.mem {self._expr(expr.left)} {self._expr(expr.right)}"
            if op == 'and': op_str = '&&'
            elif op == 'or': op_str = '\\/'
            elif op == 'not': op_str = '~'
            elif op == '==': op_str = '='
        ...
        if isinstance(expr, UnaryOp):
            ...
            # Wrap complex inner exprs in parens to fix precedence
            if isinstance(expr.expr, (App, BinOp)):
                inner = f'({inner})'
            return f"~{inner}" if expr.op == 'not' else f"{expr.op}{inner}"
        ...
        if isinstance(expr, Match):
            cases = '\n        | '.join(
                f"{self._pattern(c.pattern)} -> {self._expr(c.expr)}" for c in expr.cases)
        ...
        if isinstance(expr, ArrayIndex):
            return f"List.nth {self._expr(expr.arr)} {self._expr(expr.index)}"
